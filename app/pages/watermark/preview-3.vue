<template>
  <div class="position-relative watermark-page-container">
    <!-- ลายน้ำหน้าเว็บ แบบสุ่มตำแหน่ง (สีฟ้อนต์โปร่งใส) -->
    <div
    ...
    <div
      ...
      style="z-index: 1;"
    >
      ตัวอย่างลายน้ำ - วิธีที่ 3 ลายน้ำข้อความโปร่งใสแบบวุ้น (Jelly / Glass Effect)
    </div>
...
const generateRandomWatermarks = () => {
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      const opacity = 0.4 + Math.random() * 0.15;
...
.watermark-item > div {
  ...
  font-weight: 900;
  display: block;
  /* เอฟเฟกต์ข้อความโปร่งใสแบบวุ้น/แก้วใส (Glassmorphic / Jelly Text) */
  color: transparent;
  -webkit-text-stroke: 1.2px rgba(90, 90, 90, 0.4);
  text-shadow:
